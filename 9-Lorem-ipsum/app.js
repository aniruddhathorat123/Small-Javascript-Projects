// lorem text
// each item reprsent one paragraph.
const text = [
...
// listen submit event for form.
form.addEventListener("submit", function(e /*event object*/) {
  // default behaviour of form is to clear the form's 
  // elements fields once user submit the data.
  // So here we avoid this behaviour so that submitted
  // value will still visible to the user.
  e.preventDefault();
  const value = parseInt(amount.value);
  const randomNum = Math.floor(Math.random() * text.length);

  // if anount is empty OR contains negative number
  // OR >9 number, then just show 1 random pargraph.
  if (isNaN(value) || value <= 0 || value > 9) {
    result.innerHTML = `<p class="result">${text[randomNum]}</p>`;
  } else {
    // tempText contains slice of array and each element
    // in temptext contains one paragraph in form of one 
    // string.
    let tempText = text.slice(0, value);
    // for each string paragraph in tempText, combine 
    // all the string paragraph into the one string of 
    // multiple paragraph.
